...
const CONFIG = {
  ...
  //
  // Lowered from 100,000 to 30,000: the Workers Free plan allows only 10 ms of
  // CPU per request, and 100,000 iterations blew past it. Cloudflare then killed
  // the request with Error 1102, whose response carries no CORS headers, so the
  // browser reported it as "Could not reach the server" on signup, login and
  // password reset. OTP hashing already used 10,000 and never hit this.
  // On the Workers Paid plan this can go back up to 100,000 or higher.
  ...
  // Used when the MAIL_FROM variable is missing. Deploying from GitHub deletes
  // plain-text dashboard variables, and Resend's fallback test sender
  // (onboarding@resend.dev) can only email the Resend account owner, so a lost
  // MAIL_FROM silently broke OTP for everyone else. docbrisk.com is verified
  // in Resend, so this is a safe default.
  DEFAULT_MAIL_FROM: 'DocBrisk <no-reply@docbrisk.com>',
  // Owner accounts that always get Pro, even if a GitHub deploy wipes the
  // OWNER_EMAILS variable. OWNER_EMAILS (if set) is added on top of these.
  // Each email still needs a DocBrisk account and its own password to sign in,
  // so listing it here grants nothing to anyone who does not own the account.
  DEFAULT_OWNER_EMAILS: [
    'nitish.choudhary.office@gmail.com',
    'nitishchoudhary.ecofriendly@gmail.com'
  ]
};

/* The sender address and payment gateway, with safe defaults if a deploy
   wiped the plain-text variables. Secrets (API keys) are never wiped. */
const mailFrom = (env) => env.MAIL_FROM || CONFIG.DEFAULT_MAIL_FROM;
const gatewayProvider = (env) => (env.GATEWAY_PROVIDER || (env.GATEWAY_SECRET ? 'razorpay' : '')).toLowerCase();
...
async function sendEmailResend(env, to, subject, html) {
  ...
  if (!r.ok) {
    ...
    // Resend's test mode: an unverified sender domain only reaches the Resend account owner.
    if (/testing emails|verify a domain|domain is not verified|not verified/i.test(t)) {
      throw new Error('resend test mode: the sender domain of "' + mailFrom(env) + '" is not verified in Resend, so it only delivers to ' +
        'the Resend account owner. Verify the domain at resend.com/domains. Resend said: ' + t.slice(0, 160));
    }
    throw new Error('resend: ' + t.slice(0, 180));
  }
}
...
/* Send with the configured email provider. If it fails and the other
   provider also has an API key, try that one before giving up. */
async function sendEmail(env, to, subject, html) {
  const primary = (env.EMAIL_PROVIDER || 'resend').toLowerCase() === 'brevo' ? 'brevo' : 'resend';
  const order = primary === 'brevo' ? ['brevo', 'resend'] : ['resend', 'brevo'];
  const send = { resend: sendEmailResend, brevo: sendEmailBrevo };
  const hasKey = { resend: !!env.RESEND_API_KEY, brevo: !!env.BREVO_API_KEY };
  const errors = [];
  for (const p of order) {
    if (!hasKey[p]) { if (p === primary) errors.push(p + ': API key is not set'); continue; }
    try { await send[p](env, to, subject, html); return p; }
...
async function deliverOtp(env, channel, destination, code, purpose) {
  if (channel === 'email') {
    return sendEmail(env, destination, 'Your DocBrisk verification code', otpEmailHtml(code, purpose));
  }
...
function ownerSet(env) {
  if (!OWNER_SET) {
    const fromVar = String(env.OWNER_EMAILS || '').split(',');
    OWNER_SET = new Set(CONFIG.DEFAULT_OWNER_EMAILS.concat(fromVar)
      .map(e => String(e).trim().toLowerCase()).filter(Boolean));
  }
  return OWNER_SET;
}
function isOwner(env, email) {
  return ownerSet(env).has(String(email || '').trim().toLowerCase());
}
...
async function deliverProEmail(env, u) {
  return sendEmail(env, u.email, 'DocBrisk Pro is active', proActiveEmailHtml(u.name));
}
...
async function handle(request, env, ctx) {
  ...
  if (path === '/api/health') {
    // Open this URL after every deploy: "warnings" lists anything missing.
    // Nothing secret is shown, only whether each setting is present.
    const warnings = [];
    const provider = (env.EMAIL_PROVIDER || 'resend').toLowerCase();
    if (provider === 'resend' && !env.RESEND_API_KEY) warnings.push('RESEND_API_KEY is missing: no OTP emails can be sent.');
    if (provider === 'brevo' && !env.BREVO_API_KEY) warnings.push('BREVO_API_KEY is missing: no OTP emails can be sent.');
    if (!env.MAIL_FROM) warnings.push('MAIL_FROM is not set; using ' + CONFIG.DEFAULT_MAIL_FROM + '. Add it as a Secret so deploys cannot remove it.');
    if (/resend\.dev/i.test(mailFrom(env))) warnings.push('The sender is a resend.dev test address: Resend only delivers it to your own account email.');
    if (!env.GATEWAY_SECRET) warnings.push('GATEWAY_SECRET is missing: Razorpay payments will not activate Pro automatically.');
    if (!ownerSet(env).size) warnings.push('No owner accounts: set OWNER_EMAILS or CONFIG.DEFAULT_OWNER_EMAILS.');
    // /api/health?check=email also asks Resend whether the sender's domain is verified.
    let emailCheck;
    if (url.searchParams.get('check') === 'email' && env.RESEND_API_KEY) {
      const domain = ((mailFrom(env).match(/@([^>\s]+)/) || [])[1] || '').toLowerCase();
      try {
        const r = await fetch('https://api.resend.com/domains', { headers: { 'Authorization': `Bearer ${env.RESEND_API_KEY}` } });
        const d = await r.json().catch(() => ({}));
        if (r.ok) {
          const list = (d.data || []).map(x => ({ name: x.name, status: x.status }));
          const hit = list.find(x => domain === x.name || domain.endsWith('.' + x.name));
          emailCheck = { senderDomain: domain, status: hit ? hit.status : 'not added to Resend', domainsInResend: list };
          if (!hit || hit.status !== 'verified') warnings.push('Resend reports ' + domain + ' as "' + emailCheck.status + '": OTPs only reach the Resend account owner until it is verified.');
        } else {
          emailCheck = { senderDomain: domain, status: 'unknown: this Resend API key can only send; check resend.com/domains' };
        }
      } catch (e) { emailCheck = { senderDomain: domain, status: 'check failed: ' + String(e.message).slice(0, 80) }; }
    }
    return json({
      emailCheck,
      ...
      paymentGateway: gatewayProvider(env) || 'none',
      paymentGatewayConfigured: !!(gatewayProvider(env) && env.GATEWAY_SECRET),
      ownerAccounts: ownerSet(env).size,
