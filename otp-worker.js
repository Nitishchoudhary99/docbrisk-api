/* ==========================================================================
   ...
   This is the piece a browser cannot do on its own. Everything security-
   sensitive lives here: provider credentials, the password database, OTP
   secrets, and the decision about who is actually a paying customer.

   Endpoints
     POST /api/otp/send              { channel, destination, purpose }
     POST /api/otp/verify            { channel, destination, purpose, code } -> ticket
     POST /api/auth/signup           { name, email, phone, password, ticket? }
     POST /api/auth/login            { email, password }
     GET  /api/auth/me               Bearer
     POST /api/auth/password         Bearer { currentPassword, newPassword }
     POST /api/auth/reset/start      { identifier, channel }
     POST /api/auth/reset/complete   { email, ticket, newPassword }
     POST /api/billing/claim         Bearer { utr }
     GET  /api/billing/status        Bearer
     POST /api/admin/payments        x-admin-key           (list pending)
     POST /api/admin/verify-payment  x-admin-key { email, utr, approve }

   Storage: one Workers KV namespace. Keys:
     u:<email>          user record
     p:<phone>          -> email (lookup)
     otp:<purpose>:<dest>   hashed code + attempts (TTL 10 min)
     tk:<ticket>        proof that an OTP was verified (TTL 15 min)
     rl:<bucket>        rate-limit counter (TTL window)
   ========================================================================== */
...
/* ---------------------------------------------------------------- providers */
/** Email via Resend (free tier, no card). Set EMAIL_PROVIDER=resend. */
...
/** Email via Brevo (300/day free). Set EMAIL_PROVIDER=brevo. */
...
/** SMS via MSG91 — the practical choice for Indian numbers. */
...
/** SMS via Twilio — works worldwide. */
...
function proActiveEmailHtml(name) {
  return `<div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;max-width:460px;margin:0 auto;padding:28px">
    ...
    <p style="color:#475569;font-size:14px">That unlocks all 25 CV templates, the Watermark Remover, Batch Processor, Smart Redaction, Extract Images, Clean Scan, Resize Pages and the QR Stamper.</p>
...
function publicUser(u) {
  return {
    ...
    plan: proActive(u) ? 'pro' : 'free',
    renews: u.renews || null,
...
async function otpVerify(env, body, ip) {
  ...
  // hand back a short-lived ticket proving this destination was verified
...
/* ---------------------------------------------------------------- gateway webhook
   When you do get a payment gateway, point its webhook here and payments
   verify themselves — no manual approval. Supported out of the box:
   Razorpay and Cashfree. The shared secret guards against forged calls. */
...
async function handle(request, env, ctx) {
  ...
  if (path === '/api/auth/signup' && request.method === 'POST') {
    ...
    // require a verified email ticket unless the operator disables it
    ...
    const recovery = [];
    for (const r of recoveryPlain) recovery.push(await pbkdf2(r, salt, 20000));
    ...
    return json({ token: await issueSession(env, u), user: publicUser(u), recoveryCodes: recoveryPlain });
  }
  ...
  if (path === '/api/auth/login' && request.method === 'POST') {
    ...
    return json({ token: await issueSession(env, u), user: publicUser(u) });
  }
  ...
  if (path === '/api/billing/status') {
    const u = await requireAuth(env, request);
    if (!u) return json({ error: 'Not signed in.' }, 401);
    return json({ pro: proActive(u), user: publicUser(u) });
  }
  ...
  if (path.startsWith('/api/admin/')) {
    ...
    if (path === '/api/admin/verify-payment' && request.method === 'POST') {
      ...
      await putUser(env, u);
      if (body.approve !== false) {
        // best effort — never fail the approval because email bounced
